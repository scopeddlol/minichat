...
export const direct = {
  ...
  history: (id: string, before?: string) =>
    request<DirectMessage[]>(
      `/direct/${id}/messages${before ? `?before=${encodeURIComponent(before)}` : ''}`,
    ),
...
gateway.onStatus((status) => {
  if (status === 'closed') useInbox.setState({ drafts: {}, open: false, active: null, conversations: [] })
})
gateway.on((event) => {
  if (['READY', 'DIRECT_MESSAGE', 'DIRECT_READ'].includes(event.t))
    void useInbox
      .getState()
      .refresh()
      .catch(() => undefined)
  if (event.t === 'INVALID_SESSION')
    useInbox.setState({ drafts: {}, open: false, active: null, conversations: [] })
})
