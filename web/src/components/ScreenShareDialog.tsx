import { hasNativeCapture } from '../lib/nativeCapture'
    ...
    >
      <div className="px-5 py-4 space-y-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            {RESOLUTIONS.map((option) => (
              <button
                ...
                style={{
                  borderColor:
                    settings.resolution === option.value ? 'var(--accent)' : 'var(--border)',
                  background:
                    settings.resolution === option.value ? 'var(--accent-soft)' : 'transparent',
                  color:
                    settings.resolution === option.value ? 'var(--accent)' : 'var(--text-muted)',
                }}
              >
                {option.label}
              </button>
            ...
            {(
              [
                [
                  'motion',
                  'Motion',
                  'Games and video — keeps frames smooth',
                  <MonitorPlay size={14} key="m" />,
                ],
                [
                  'detail',
                  'Detail',
                  'Code and text — keeps the picture sharp',
                  <Type size={14} key="d" />,
                ],
              ] as const
              ...
              >
                <span
                  ...
                  style={{
                    color: settings.content === value ? 'var(--accent)' : 'var(--text)',
                  }}
                >
                  {icon} {label}
                </span>
        ...
        >
          ...
          <span>
            About <strong>{estimate} Mbps</strong> up at this setting.{' '}
            {native
              ? 'Next, choose a screen or window in the MiniChat desktop picker.'
              : 'Your browser will ask which screen or window to share next.'}
          </span>
        </p>
      </div>
    </Modal>
