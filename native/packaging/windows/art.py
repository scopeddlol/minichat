#!/usr/bin/env python3
"""Regenerate the native client's Windows installer artwork from its icon.
...
the icon or the wordmark does not leave four stale bitmaps behind.
    ...
    python3 native/packaging/windows/art.py
...
ROOT = NATIVE.parent
ICON = NATIVE / "assets" / "icons" / "icon.png"
...
def panel(canvas: Image.Image, width: int, height: int, scale: float = 1.0) -> None:
    ...
    for index, line in enumerate(("Voice and text chat", "for one community")):
        centred(
            draw,
            line,
...
def main() -> None:
    for name, build in (
        ...
        image = build()
        # 24-bit, bottom-up, 40-byte header: what NSIS reads. A bit depth
        # with an alpha channel is what makes it render a black rectangle
        # instead.
